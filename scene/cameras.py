...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, image_name, invdepthmap, depth_params,
                 resolution_scale, uid, trans=np.array([0.0, 0.0, 0.0]), 
                 scale=1.0, data_device = "cuda", data_type=None,
                 ):
        ...
        if image.shape[0] == 4:
            self.alpha_mask = image[3:4, ...].to(self.data_device)
        else:
