...
class BasicModel:
    ...
    # statis grad information to guide lifting.
    def training_statis(self, render_pkg, width, height, data_type=None, density_split=False):
        ...
        # lazy update
        if data_type in ["aerial", "v1"] and hasattr(self, "offset_gradient_accum_aerial"):
            self.offset_gradient_accum_aerial[combined_mask] += grad_norm
            self.offset_denom_aerial[combined_mask] += 1
        elif data_type in ["street", "v2"] and hasattr(self, "offset_gradient_accum_street"):
            self.offset_gradient_accum_street[combined_mask] += grad_norm
            self.offset_denom_street[combined_mask] += 1
        elif density_split:
            raise NotImplementedError("Density split requires split statis!")
    ...
    def clean(self):
        ...
        if hasattr(self, 'offset_gradient_accum_aerial'):
