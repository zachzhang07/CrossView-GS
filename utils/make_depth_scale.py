...
def get_scales(key, cameras, images, points3d_ordered, args):
    ...
    n_remove = len(name.split('.')[-1]) + 1
    invmonodepthmap = cv2.imread(f"{args.depths_dir}/{name[:-n_remove]}.png", cv2.IMREAD_UNCHANGED)
    ...
    if valid.sum() > 10 and (invcolmapdepth.max() - invcolmapdepth.min()) > 1e-3:
        ...
        scale = 0
        offset = 0
    return {"image_name": name[:-n_remove], "scale": scale, "offset": offset}
