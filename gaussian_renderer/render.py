...
def render(viewpoint_camera, pc, pipe, bg_color, iteration, render_mode, ape_code=-1):
    ...
    xyz, color, opacity, scaling, rot, sh_degree, selection_mask = pc.generate_neural_gaussians(viewpoint_camera,
                                                                                                visible_mask, ape_code)
...
def prefilter_voxel(viewpoint_camera, pc, pipe, bg_color):
    ...
    # tmp = ((radii.squeeze(0) > 0) & (radii.squeeze(0) < int(torch.quantile(radii.squeeze(0).float(), 0.999))))
    # visible_mask[pc._anchor_mask] = tmp
    # del tmp
